...
def uebersicht():
    ...
    return render_template("mod_atemschutz/index.html", user=auth_module.get_userobject(),
                           ...
                           entries=Entry.query.filter(func.DATE(Entry.datum) > date_lastYear).all(),
...
def settings():
    return render_template("mod_atemschutz/settings.html", user=auth_module.get_userobject(),
...
def entry_AutoNew(firefighter_id, datum, kurs):
    db_category = Category.query.filter_by(name="Kurs").first()
    firefighter = Firefighter.query.filter_by(id=firefighter_id).first()
    if kurs.as_time > 0:
    ...
    notifications.create("AS_general", "Neuer AS Eintrag", "%s %s (%s) %s min" % (firefighter.vorname,firefighter.name,kurs.name,kurs.as_time), "/atemschutz")
