...
from app import db, oidc, auth_module, mail
...
class notifications:
    ...
    def send_email(subject, recipient, title, message, url):
        # Check if the URL is absolut or relativ
        if "http" not in url:
            url = app.config["URL"] + url

        msg = Message("[FW-Effi] %s" % subject, sender=app.config['MAIL_USERNAME'], recipients=[recipient])
        msg.body = render_template("mod_core/mail_newNotification.txt", title=title, message=message, url=url)
        msg.html = render_template("mod_core/mail_newNotification.html", title=title, message=message, url=url)
        mail.send(msg)
    ...
    def create(sender_type, title, message, url):
        ...
        for receiver in category.recipients:
            db_message = notifications_messages(
                receiver.id,
            ...
            notifications.send_email(title, receiver.email, title, message, url)
...
def post_permission():
    try:
        ...
        for key in req:
            group = re.search('''(?<=')\s*[^']+?\s*(?=')''',
                              key).group()  # Extract Group Name from "roles['<groupName>']"

            auth_group = db.session.query(auth_groups).filter_by(
                name=group).first()  # Find auth_group DB Entry with that name
...
def post_notifications():
    try:
        ...
        print(req)
        for key in req:
            category = re.search('''(?<=')\s*[^']+?\s*(?=')''',
                                 key).group()  # Extract Category Name from "roles['<categoryName>']"

            notification_category = db.session.query(notifications_category).filter_by(
                name=category).first()  # Find Notification Category DB Entry with that name
